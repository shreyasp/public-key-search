...
class UserController extends Controller
{
  ...
  public function authenticateUser(Request $request){
    ...
    if(password_verify($password, $_password)) {
      // If it's a match :) .
      // Proceed ahead by updating sessionUser cookie and Session ID in redis and DB.
      $cookie = new Cookie('sessionUser', $email);
      $sessionId = bin2hex(random_bytes(16));

      Cache::forever($email, $sessionId);
      $record->sessionId = $sessionId;

      $record->save();
      $firstName = strtolower($record->firstName);

      return redirect('main/'.$firstName)->withCookie($cookie);
    }
    // Redirect to index on wrong password input
    else{
      return redirect('/');
    }
  }
